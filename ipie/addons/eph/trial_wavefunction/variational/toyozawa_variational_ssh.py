...
from ipie.addons.eph.trial_wavefunction.variational.coherent_state_variational_ssh import local_energy_acoustic 

import jax
import jax.numpy as npj


def gradient_toyozawa_mo(x, hamiltonian, system, perms, coeffs):
    grad = np.array(
        jax.grad(objective_function_toyozawa_mo)(
            x, hamiltonian, system, perms, coeffs
        )
    )
    return grad

def objective_function_toyozawa_mo(hamiltonian, system):
...
def objective_function_toyozawa_mo(x, hamiltonian, system, perms, coeffs, zero_th=1e-12):
...
#        jax.debug.print('energy:    {x}', x=ke + pe + e_ph + e_eph)
        ...
        kinetic = np.sum(hamiltonian.T[0] * G_j[0] + hamiltonian.T[1] * G_j[1])
        
        X = (1/npj.sqrt(2*hamiltonian.m*hamiltonian.w)) * (beta0 + beta_j)
        ...
        tmp0 = hamiltonian.g_tensor * G_j[0] * displ_mat
        if system.ndown > 0:
            tmp0 += hamiltonian.g_tensor * G_j[1] * displ_mat
        el_ph_contrib = 2 * jax.numpy.sum(tmp0)

        phonon_contrib = hamiltonian.w * jax.numpy.sum(beta0 * beta_j)
...
#        num_energy += local_energy_acoustic()

        num_energy += (kinetic + el_ph_contrib + phonon_contrib) * overlap  # 2.0 comes from hermiticity
        denom += overlap
...
#            coeffs,
 #           False,
 #       ),
        args=(hamiltonian, system, perms, coeffs),
