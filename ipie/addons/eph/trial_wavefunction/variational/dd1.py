...
class dD1Variational(ToyozawaVariational):
    ...
    def gradient(self, x, *args) -> np.ndarray:
        ...
        for ip, (permi, coeffi) in enumerate(zip(self.perms, self.Kcoeffs)):
            ...
            d_shift_perm_r = np.einsum(
                "ei,pj->peij", np.eye(self.ham.N), perm_mat.dot(shift).dot(perm_mat.T)
            )
            d_shift_perm_r += np.einsum("pi,je->peij", perm_mat.T.dot(shift.conj()), perm_mat)
            #            d_shift_perm_r = np.zeros_like(d_shift_perm_r)

            d_shift_perm_i = -1j * np.einsum(
                "ei,pj->peij", np.eye(self.ham.N), perm_mat.dot(shift).dot(perm_mat.T)
            )
            ...
            d_cs_ovlp_r -= 0.5 * np.einsum(
                "pb,ej,jb,ij->peij", shift, perm_mat.T, perm_mat, cs_ovlp
            )
            d_cs_ovlp_r -= 0.5 * np.einsum(
                "pb,bj,je,ij->peij", shift.conj(), perm_mat.T, perm_mat, cs_ovlp
            )
            ...
            d_cs_ovlp_i += (
                1j * 0.5 * np.einsum("pb,ej,jb,ij->peij", shift, perm_mat.T, perm_mat, cs_ovlp)
            )
            d_cs_ovlp_i -= (
                1j
                * 0.5
                * np.einsum("pb,bj,je,ij->peij", shift.conj(), perm_mat.T, perm_mat, cs_ovlp)
            )  # TODO
            ...
            el_ph_contrib = np.einsum(
                "ijk,ki,peij->pe", elph_ij_contracted, shift.conj(), d_cs_ovlp_r
            )
            ...
            el_ph_contrib += np.einsum(
                "ijk,ij,kp,ej->pe", elph_ij_contracted, cs_ovlp, perm_mat, perm_mat.T
            )
            ...
            el_ph_contrib = np.einsum(
                "ijk,ki,peij->pe", elph_ij_contracted, shift.conj(), d_cs_ovlp_i
            )  # TODO contract over k already to 2 index object
            ...
            ph_energy = self.ham.w0 * npj.einsum(
                "ij,j->", shift.conj() * beta_i, cs_ovlp.diagonal() * Ga_i.diagonal()
            )
